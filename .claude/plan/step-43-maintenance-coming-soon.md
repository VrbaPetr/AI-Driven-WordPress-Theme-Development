...
- Call `wp_loaded` is not needed — WordPress has already bootstrapped by the time `template_redirect` fires, so all WP functions including `get_field()` are available.
